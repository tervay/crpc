...
public:
  ...
  Response send(
    ...
    return send(method, std::vector<uint8_t>(body.begin(), body.end()), headers,
                timeout);
  }
        ...
        for (;;) {
          ...
          if (line.empty()) {
            ...
            break;
                     ...
                     State::parsingStatusLine) // RFC 7230, 3.1.2. Status Line
          {
            state = State::parsingHeaders;

            const auto httpEndIterator =
              ...
              if (statusEndIterator != line.end()) {
                ...
                response.description =
                    std::string{descriptionStartIterator, line.end()};
                     ...
                     State::parsingHeaders) // RFC 7230, 3.2. Header Fields
          {
            ...
            headerValue.erase(headerValue.begin(),
                              std::find_if(headerValue.begin(),
                                           headerValue.end(), isNotWhitespace));

            // rtrim
            headerValue.erase(std::find_if(headerValue.rbegin(),
                                           headerValue.rend(), isNotWhitespace)
                                  .base(),
                              headerValue.end());
